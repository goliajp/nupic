...
fn signals(raw: &[u8], w: u32) -> (f64, f64, f64, f64, usize) {
    ...
    for px in raw.chunks_exact(4) { if px[3] == 255 { n_opaque += 1; } }
    ...
    let w = w as usize; let h = n_total / w;
    let target_rows = (500_000 / (w-1).max(1)).max(1);
...
fn main() -> anyhow::Result<()> {
    for f in env::args().skip(1) {
        let img = ImageReader::open(&f)?.with_guessed_format()?.decode()?;
        let r = img.to_rgba8();
        let w = r.width();
        let raw = r.into_raw();
        let (opq, mr, am, av, uniq) = signals(&raw, w);
        let fname = std::path::Path::new(&f).file_name().unwrap().to_string_lossy().to_string();
        println!("{:<55} opq={:.3} mr={:.2} adj_mn={:.2} var={:.1} uniq={}",
            fname, opq, mr, am, av, uniq);
    }
    Ok(())
}
