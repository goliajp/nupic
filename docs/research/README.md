...
- [`png/05-nupic-bits-stage-0.md`](png/05-nupic-bits-stage-0.md) — **`nupic-bits` stage-0 graduation**(roadmap 阶段 0,memory 原 0.5.x 计划)。CRC-32 + Adler-32 + LSB-first bit I/O,**0 runtime deps**;`#![no_std]`;CRC 2.44 GB/s(crc32fast 10.37 GB/s 用 SIMD pclmul,4× faster — S0.1 NEON intrinsics polish backlog);Adler 4.18 GB/s ≈ cement 4.32 GB/s parity;13 测 + RFC 1950/1952 fixtures + crc32fast/adler32 oracle 跨 600 长度全过;bit I/O property 测帮 catch 一个 1u8<<8 overflow bug。**Stage 1 nupic-deflate unblocked**。
- [`png/06-nupic-deflate-design.md`](png/06-nupic-deflate-design.md) — **`nupic-deflate` stage 1 设计 anchor**(roadmap 阶段 1)。DEFLATE format 拆解 + 6 phase plan(1.0 greedy + static Huffman → 1.2 lazy match graduation → 1.4 zopfli-class)。Ceiling 数据:zlib level 1 ~0.50 raw,zopfli ~0.34 raw;encode time 跨 phase 估算。Graduation = phase 1.2 ≤ 1.05× zopfli output,encode ≥ zlib level 6。Mem ceiling 192 KiB working set regardless of input size。Stage 1.0 起手实现是 06-bis 候选(下一篇);本篇纯 design。
- [`png/06-bis-deflate-stored-blocks.md`](png/06-bis-deflate-stored-blocks.md) — **nupic-deflate phase 1.0.0** infrastructure。Stored-block-only DEFLATE encoder + zlib wrapper(CMF / FLG / Adler-32 via nupic-bits)。No compression yet,output `~ 1.0005×` raw + 5 byte/block header。16 tests pass:flate2 / miniz_oxide roundtrip across empty / 1 byte / 65 536-byte block boundary / 200 000-byte multi-block 全过;zlib FCHECK / Adler validated。Phase 1.0.1(LZ77 + static Huffman,real compression)是 06-ter。
- [`png/06-ter-deflate-lz77.md`](png/06-ter-deflate-lz77.md) — **nupic-deflate phase 1.0.1**:LZ77 greedy hash chain + RFC 1951 §3.2.6 fixed Huffman block。**Beats zlib level 1 on 4/4 inputs**:repeats-10k → 0.66× L1, text → 0.80×, random / PNG → 1.00× (持平)。 graduation target "zlib L1 class" met。`Level::Fast` 是新 default。17 tests pass(含 fast-path repeats / text / random)。`tables.rs` const-fn-generated fixed Huffman lookup + length/distance encoding tables。256 KiB working set(2 hash arrays × 32 K × u32)。L9 gap (repeats 2.39×, text 1.33×) → phase 1.0.2 dynamic Huffman 计划。
